...
public class TooManyFuncts2
{
    ...
    /// <param name="isGOInActive">checks GameObject.activeSelf || null, if no filter | true - must be active, false - must be inactive</param>
    /// <param name="isCompEnDisabled">checks Component(Behaviour to be exact).enabled</param>
    /// <returns>returns T</returns>
#nullable enable
    public static T? GetComponentInChildrenParametric<T>(Transform parent, string? nameContaining, string? tag, bool? isGOInActive, bool? isCompEnDisabled) where T : Behaviour
    {
        T comp;
        foreach (Transform c in parent)
        {
            ...
            if (tag is string && !c.gameObject.CompareTag(tag))
                continue;
            if (isGOInActive is bool && isGOInActive != c.gameObject.activeSelf)
                continue;

            comp = c.GetComponentInChildren<T>(true);
            if (comp is null)
                continue;
            if (isCompEnDisabled is bool && isCompEnDisabled != comp.enabled)
                continue;
...
#nullable enable
    public static List<T>? GetComponentsInChildrenParametric<T>(Transform parent, string? nameContaining, string? tag, bool? isGOInActive, bool? isCompEnDisabled) where T : Behaviour
    {
        List<T> res = new();
        List<T> comps = new();
        foreach (Transform c in parent)
        {
            ...
            c.GetComponentsInChildren<T>(true, comps);
            if (comps.Count == 0)
                continue;
            foreach (T behaviour in comps)
                if (isCompEnDisabled is null || isCompEnDisabled == behaviour.enabled)
                    if (isGOInActive is null || isGOInActive == behaviour.gameObject.activeSelf)
                        res.Add(behaviour);
            //res.AddRange(comps);
        }
        if (res.Count > 0)
